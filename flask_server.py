...
def editItem(category_id, item_id):
    ...
    creator = getUserInfo(category.user_id)
    if 'username' not in login_session or category.user_id != login_session['user_id']:
    	return "<script>function myFunction() {alert('You are not the owner of this item');} </script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
...
def deleteItem(category_id, item_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteItem.html', item_id=item_id, category_id=category_id, item=itemToDelete)
